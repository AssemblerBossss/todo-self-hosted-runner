...
class OpenRouterService:
    ...
    async def generate_title(self, details: str, current_title: str | None = None) -> str:
        ...
        user_prompt = (
            ...
            f"Описание заметки:\n{details.strip()}\n\n"
            "Сгенерируй лучший заголовок, который четко поясняет суть заметки, чтобы прочитавшему заголовок человека сразу стало понятно, о чем речь."
        )
    ...
    async def suggest_tag(
        ...
        system_prompt = (
            ...
            "Верни только один тег из 1 слова, без кавычек и без пояснений. Тег поясняет суть заметки в одном слове."
        )
        user_prompt = (
            f"Заголовок заметки: {title or 'нет'}\n"
            f"Описание заметки:\n{(details or '').strip()}\n\n"
            f"Существующие теги в системе: {', '.join(existing_tags) if existing_tags else 'нет'}\n"
            f"Похожие заметки из того же кластера (кластер формирует наиболее схожие заметки, поэтому можно опираться на теги в кластере для генерации нового):\n{cluster_context}\n\n"
            "Предложи лучший тег."
        )
        return await self._complete(system_prompt, user_prompt)
